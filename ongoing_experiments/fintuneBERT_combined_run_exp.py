# run_all_exp_combined_data_single.py
"""
run_all_exp_combined_data_single.py

This script runs a series of single-run experiments (15 configurations) on the combined dataset
using the function train_eval_single_combined from single_run_combined.py. It applies different preprocessing
and model settings and logs FeReRe metrics via MLflow.

RESULT FILES SAVED:
  For each experiment (identified by its exp_id):
    - Classified Feedback (wide-format):
         classified_feedback_requirements_single_exp_<exp_id>.xlsx
    - Test Ground Truth (wide-format):
         test_ground_truth_single_exp_<exp_id>.xlsx
  These files are saved in the results directory (e.g., "./single_results").

  TensorBoard logs are stored in the log directory (e.g., "./logs_single").
...
import random
import mlflow
from fintuneBERT_combined import train_eval_single_combined
import pandas as pd
from pathlib import Path

# -------------------------------
# incorporate_previously_assigned_feedback_single Function
# -------------------------------
def incorporate_previously_assigned_feedback_single(
    feedback_file, 
    requirements_file, 
    gt_file, 
    ...
    """
    print("[SINGLE RUN] Reading ground truth (wide format):", gt_file)
    gt_df = pd.read_excel(gt_file, header=None)
    print("[SINGLE RUN] Reading previously assigned feedback (wide format):", previously_assigned_file)
    ...
    expanded_gt = tmp_dir / "expanded_ground_truth_single.xlsx"
    merged_df.to_excel(expanded_gt, header=False, index=False)

    print("[SINGLE RUN] Merged ground truth with previously assigned feedback saved to:", expanded_gt)
...
#best_preprocessing = {"remove_sw": False, "augment": True, "do_split": True}
best_preprocessing = {"remove_sw": False, "augment": True, "do_split": True}
...
experiments = [
    ...
    {"exp_id": 8,  "remove_sw": False, "augment": False, "do_split": False, "epochs": 20, "model_name": "roberta-base"},
    {"exp_id": 9,  "remove_sw": best_preprocessing["remove_sw"], "augment": best_preprocessing["augment"], "do_split": best_preprocessing["do_split"], "epochs": 20, "model_name": "roberta-base"},
    {"exp_id": 10, "remove_sw": False, "augment": False, "do_split": False, "epochs": 20, "model_name": "bert-large-uncased"},
    {"exp_id": 11, "remove_sw": best_preprocessing["remove_sw"], "augment": best_preprocessing["augment"], "do_split": best_preprocessing["do_split"], "epochs": 20, "model_name": "bert-large-uncased"},
    {"exp_id": 12, "remove_sw": False, "augment": False, "do_split": False, "epochs": 20, "model_name": "distilbert-base-uncased"},
    {"exp_id": 13, "remove_sw": best_preprocessing["remove_sw"], "augment": best_preprocessing["augment"], "do_split": best_preprocessing["do_split"], "epochs": 20, "model_name": "distilbert-base-uncased"},
    #Exp_15 is the baseline exp
    {"exp_id": 15, "remove_sw": False, "augment": False, "do_split": False, "epochs": 20, "model_name": "bert-base-uncased"},

    # # Experiment 14: Incorporate previously assigned feedback (using best_preprocessing)
    {"exp_id": 14, "remove_sw": best_preprocessing["remove_sw"], "augment": best_preprocessing["augment"], "do_split": best_preprocessing["do_split"], "epochs": 20, "model_name": "bert-large-uncased"}
]

# Optionally, set a common MLflow experiment name for all single-run experiments
mlflow.set_experiment("FeReRe_Single_Combined_Experiments")

def run_all_experiments():
    for exp in experiments:
        print("\n==========================================")
        print(f"Running Single-Run Experiment {exp['exp_id']} with parameters:")
        print(f"  remove_sw={exp['remove_sw']} | augment={exp['augment']} | do_split={exp['do_split']}")
        print(f"  epochs={exp['epochs']} | model_name={exp['model_name']}")
        print("==========================================\n")
        
        if exp["exp_id"] == 14:
            # For Experiment 14, choose a random previously assigned file from one earlier experiment’s output.
            
            prev_assigned = f"/nfs/home/vthakur_paech/single_results/classified_feedback_requirements_single_exp_10.xlsx"
            
            f_path, r_path, g_path = incorporate_previously_assigned_feedback_single(
                feedback_files[0],
    ...
    print("\n[ALL SINGLE-RUN EXPERIMENTS COMPLETED]\n")
