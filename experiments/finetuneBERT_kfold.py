...
def train_and_eval_kfold(feedback, requirements, ground_truth, epochs=1, n_splits=5):
    nltk.download('punkt')
    ...
    fold = 1
    for train_indices, test_indices in kfold.split(input_ids,np.array(labels)):
        ...
        pd.DataFrame(prediction).to_excel(f'../data/finetuneBERT/kfold/predictions_{fold}.xlsx', index=False)
        ...
        print(f'Fold {fold} - Precision: {precision}, Recall: {recall}, F2 Score: {f2}, Avg Assign: {avg_feedback_per_requirement}')

        # Save classified feedback for each fold
        ...
        results_df.to_excel(f'../data/finetuneBERT/kfold/classified_feedback_requirements_{fold}.xlsx', index=False)

        # Save ground truth for the test split for each fold
        ...
        test_ground_truth_df.to_excel(f'../data/finetuneBERT/kfold/test_ground_truth_{fold}.xlsx', index=False)

        fold += 1


# Example usage
#train_and_eval_kfold("../data/smartage/SmartAgeSV_Feedback.xlsx", "../data/smartage/SV_issues.xlsx", "../data/smartage/SmartAgeSV_GT_formatted.xlsx", 3, 5)
#train_and_eval_kfold("../data/smartage/SmartAgeSF_Feedback.xlsx", "../data/smartage/SF_issues.xlsx", "../data/smartage/SmartAgeSF_GT_formatted.xlsx", 3, 5)
#train_and_eval_kfold("../data/komoot/AppReviews.xlsx", "../data/komoot/jira_issues_noprefix.xlsx", "../data/komoot/Komoot_Ground_Truth_ids_only.xlsx", epochs=3, n_splits=5)
train_and_eval_kfold("../data/ReFeed/feedback.xlsx", "../data/ReFeed/requirements.xlsx", "../data/ReFeed/refeed_gt.xlsx", epochs=3, n_splits=5)
