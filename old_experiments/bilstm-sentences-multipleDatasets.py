...
def load_and_sample(feedback, requirements, ground_truth):
    ...
    return all_samples, labels, positive_pairs, negative_pairs, feedback_df, requirements_df, tokenizer, vocab_size, max_length

def train_model(all_samples, labels, tokenizer, vocab_size, max_length):
    ...
    return model, X_test, y, y_test

def eval_model(ids, model, X_test,y, y_test, positive_pairs, negative_pairs, feedback_df, requirements_df):
    # Evaluate model
    ...
    results_df.to_excel(f'../data/bilstm/classified_feedback_requirements_{ids}.xlsx', index=False)

def sample_train_and_eval(feedback,requirements,ground_truth):
    # Ensure TensorFlow uses GPU
    print(f"Num GPUs Available: {len(tf.config.list_physical_devices('GPU'))}")

    dataset_info = []
    for feedback, requirements, ground_truth in zip(feedback_files, requirements_files, ground_truth_files):
        all_samples, labels, positive_pairs, negative_pairs, feedback_df, requirements_df, tokenizer, vocab_size, max_length = load_and_sample(feedback, requirements, ground_truth)
        model, X_test, y, y_test = train_model(all_samples, labels, tokenizer, vocab_size, max_length)
        dataset_info.append({
    ...
    for idx,info in enumerate(dataset_info):
        eval_model(idx, model, info['X_test'],info['y'],info['y_test'],info['positive_pairs'],info['negative_pairs'],info['feedback_df'],info['requirements_df'])



feedback_files = ["../data/smartage/SmartAgeSV_Feedback.xlsx", "../data/smartage/SmartAgeSF_Feedback.xlsx", "../data/sbert/AppReviews.xlsx"]
requirements_files = ["../data/smartage/SV_issues.xlsx", "../data/smartage/SF_issues.xlsx", "../data/sbert/jira_issues_noprefix.xlsx"]
ground_truth_files = ["../data/smartage/SmartAgeSV_GT_formatted.xlsx", "../data/smartage/SmartAgeSF_GT_formatted.xlsx", "../data/sbert/Komoot_Ground_Truth_ids_only.xlsx"]
